...
async function registerUser (caClient, wallet, orgMspId, userId, affiliation, adminUUID){
try {
  ...
  const adminIdentity = await wallet.get(adminUUID);
  if (!adminIdentity) {
  ...
  const adminUser = await provider.getUserContext(adminIdentity, adminUUID);
...
const registerUserFunction = async (userID, userName, adminUUID) => {
  // Example usage
  const ccp = buildCCPOrg1();
  const caClient = buildCAClient(FabricCAServices, ccp, 'ca.org1.example.com');
  const wallet = await Wallets.newFileSystemWallet(walletPath);
  ...
  // const patientID = 'patient2'; // Replace with actual patient ID
  try{
  await registerUser(caClient, wallet, mspOrg1, userID, 'org1.department1', adminUUID);
